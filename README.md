...
> `column` 为列索引     
> `content`为筛选项    
> `method` 为筛选的方法，如果是多选的筛选项，对每一条数据会执行多次，任意一次返回 true 就会显示。     

![FilterTable](./docs/images/filterTable.png) 
...
> `pageSizes?: Array<number>`  条数可选项，默认为`[10, 20, 50, 100]`    

![PageTable](./docs/images/pageTable.png)  
...
通过`header`属性配置表头，可配置为：首行表头、首列表头、无表头
...
![HeightTable](./docs/images/heightTable.png)  
